...
def build_dynamic_pvd_svg(
    ...
    sand_mat_L: float,
    consider_sand_mat: bool,
) -> str:
    """
    สร้างภาพ SVG แบบเคลื่อนไหวตามผลการคำนวณ

    สิ่งที่เปลี่ยนตามค่า:
    - ระดับน้ำลดลงตาม Uav
    - ความเร็วลูกศรแนวรัศมีสัมพันธ์กับ Ur
    - ความเร็วลูกศรแนวดิ่งสัมพันธ์กับ Uv
    - สีสถานะเปลี่ยนตามการผ่านเกณฑ์
    - แสดงค่าความคืบหน้าและค่า L
    """
    uav = max(0.0, min(100.0, uav_percent))
    ur = max(0.0, min(100.0, ur_percent))
    uv = max(0.0, min(100.0, uv_percent))

    passed = uav >= target_percent
    status_color = "#10B981" if passed else "#F59E0B"
    status_text = "PASS" if passed else "REVIEW"

    # ระดับน้ำเริ่มต้น y=206 และลดลงเมื่อ consolidation สูงขึ้น
    water_height = max(10.0, 75.0 * (1.0 - uav / 100.0))
    water_y = 236.0 - water_height

    # ค่า consolidation สูง = animation เร็วขึ้น
    radial_duration = max(0.65, 3.1 - (ur / 100.0) * 2.25)
    vertical_duration = max(0.70, 3.3 - (uv / 100.0) * 2.30)
    pulse_duration = max(1.1, 3.0 - (uav / 100.0) * 1.5)

    # วงแหวน progress
    radius = 42
    circumference = 2 * math.pi * radius
    progress_offset = circumference * (1.0 - uav / 100.0)

    l_text = f"{sand_mat_L:.3f}" if consider_sand_mat else "OFF"
    l_color = "#A78BFA" if consider_sand_mat else "#64748B"

    return f"""
    <svg viewBox="0 0 680 360" width="100%" xmlns="http://www.w3.org/2000/svg"
         role="img" aria-label="Dynamic PVD consolidation visualization">
      <defs>
        <linearGradient id="panelBg" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stop-color="#071426"/>
          <stop offset="55%" stop-color="#0B2039"/>
          <stop offset="100%" stop-color="#071426"/>
        </linearGradient>

        <linearGradient id="soilGrad" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="#9A643D"/>
          <stop offset="40%" stop-color="#70452F"/>
          <stop offset="100%" stop-color="#35251F"/>
        </linearGradient>

        <linearGradient id="sandGrad" x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stop-color="#E8C56D"/>
          <stop offset="50%" stop-color="#F6D98F"/>
          <stop offset="100%" stop-color="#D8AD50"/>
        </linearGradient>

        <linearGradient id="waterGrad" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="#22D3EE" stop-opacity=".72"/>
          <stop offset="100%" stop-color="#2563EB" stop-opacity=".88"/>
        </linearGradient>

        <linearGradient id="drainGrad" x1="0" y1="0" x2="1" y2="0">
        ...
        <clipPath id="soilClip">
          <rect x="34" y="104" width="612" height="210" rx="20"/>
        </clipPath>

        <style>
          .radial-arrow {{
            animation: radialFlow {radial_duration:.2f}s linear infinite;
          }}

          .vertical-arrow {{
            animation: verticalFlow {vertical_duration:.2f}s linear infinite;
          }}

          .vertical-drain-arrow {{
            animation: verticalDrainFlow {vertical_duration:.2f}s linear infinite;
          }}
          ...
          .drain-pulse {{
            animation: drainPulse {pulse_duration:.2f}s ease-in-out infinite;
          ...
          @keyframes radialFlow {{
            0%   {{ transform: translateX(-9px); opacity:.15; }}
            35%  {{ opacity:1; }}
            100% {{ transform: translateX(12px); opacity:.10; }}
          }}

          @keyframes verticalFlow {{
            0%   {{ transform: translateY(-10px); opacity:.15; }}
            35%  {{ opacity:1; }}
            100% {{ transform: translateY(12px); opacity:.12; }}
          }}

          /* การไหลใน PVD ให้เคลื่อนจากล่างขึ้นบน */
          @keyframes verticalDrainFlow {{
            0%   {{ transform: translateY(10px); opacity:.15; }}
            35%  {{ opacity:1; }}
            100% {{ transform: translateY(-12px); opacity:.12; }}
          }}
          ...
          @keyframes drainPulse {{
            0%,100% {{ opacity:.72; filter:drop-shadow(0 0 2px #22D3EE); }}
            50%     {{ opacity:1; filter:drop-shadow(0 0 9px #67E8F9); }}
          }}
      ...
      <!-- Main panel -->
      <rect x="8" y="8" width="664" height="344" rx="28"
            fill="url(#panelBg)" stroke="#1E4971" stroke-width="1.4"
            filter="url(#shadow)"/>

      <!-- Title -->
      <text x="30" y="40" fill="#F8FAFC" font-size="18"
            font-weight="900" letter-spacing=".6">
        LIVE PVD CONSOLIDATION
      </text>
      <text x="30" y="61" fill="#8EABC5" font-size="11">
        Visualization updates automatically from calculated values
      </text>

      <!-- Progress ring -->
      <g transform="translate(580 58)">
        <circle cx="0" cy="0" r="{radius}" fill="rgba(15,23,42,.72)"
                stroke="#203753" stroke-width="9"/>
        <circle cx="0" cy="0" r="{radius}" fill="none"
                stroke="{status_color}" stroke-width="9"
        ...
        <text x="0" y="-1" text-anchor="middle"
              ...
              fill="#FFFFFF" font-size="17" font-weight="900">
          {uav:.1f}%
        </text>
        <text x="0" y="17" text-anchor="middle"
              fill="#9FB1C5" font-size="8.5">
          Uav
        </text>
      </g>
      ...
      <g transform="translate(300 27)">
        <rect width="90" height="29" rx="10"
              fill="rgba(34,211,238,.10)" stroke="rgba(34,211,238,.28)"/>
        <text x="45" y="19" text-anchor="middle"
              fill="#9DEBFA" font-size="10.5" font-weight="800">
          t = {time_day:.0f} days
        </text>
      </g>

      <g transform="translate(398 27)">
        <rect width="84" height="29" rx="10"
              fill="{status_color}" fill-opacity=".12"
              stroke="{status_color}" stroke-opacity=".42"/>
        <circle class="status-pulse" cx="15" cy="14.5" r="5"
                fill="{status_color}"/>
        <text x="50" y="19" text-anchor="middle"
              fill="{status_color}" font-size="10.5" font-weight="900">
          {status_text}
        </text>
      </g>

      <!-- Soil system -->
      <rect x="34" y="88" width="612" height="36" rx="13"
            fill="url(#sandGrad)" stroke="#F6D98F" stroke-opacity=".48"/>
      <rect x="34" y="104" width="612" height="210" rx="20"
            fill="url(#soilGrad)"/>

      <!-- Soil texture -->
      <g clip-path="url(#soilClip)" opacity=".20" fill="#F7D8B1">
        <circle cx="72" cy="144" r="2"/><circle cx="115" cy="181" r="2"/>
        <circle cx="171" cy="139" r="2"/><circle cx="219" cy="205" r="2"/>
        <circle cx="270" cy="158" r="2"/><circle cx="328" cy="194" r="2"/>
        <circle cx="385" cy="151" r="2"/><circle cx="434" cy="216" r="2"/>
        <circle cx="502" cy="173" r="2"/><circle cx="561" cy="198" r="2"/>
        <circle cx="616" cy="151" r="2"/><circle cx="94" cy="252" r="2"/>
        <circle cx="188" cy="270" r="2"/><circle cx="293" cy="250" r="2"/>
        <circle cx="403" cy="270" r="2"/><circle cx="539" cy="254" r="2"/>
      </g>

      <!-- Dynamic water body -->
      <g clip-path="url(#soilClip)">
        <rect x="34" y="{water_y:.1f}" width="612" height="{water_height:.1f}"
              fill="url(#waterGrad)" opacity=".78"/>
        <path class="water-wave"
              d="M20 {water_y + 4:.1f}
                 Q65 {water_y - 4:.1f},110 {water_y + 4:.1f}
                 T200 {water_y + 4:.1f}
                 T290 {water_y + 4:.1f}
                 T380 {water_y + 4:.1f}
                 T470 {water_y + 4:.1f}
                 T560 {water_y + 4:.1f}
                 T670 {water_y + 4:.1f}"
              fill="none" stroke="#67E8F9" stroke-width="2.5" opacity=".85"/>
      </g>

      <!-- PVD drains -->
      <g filter="url(#softGlow)">
        <rect class="drain-pulse" x="118" y="113" width="10" height="181"
              rx="5" fill="url(#drainGrad)"/>
        <rect class="drain-pulse" x="228" y="113" width="10" height="181"
              rx="5" fill="url(#drainGrad)" style="animation-delay:.22s"/>
        <rect class="drain-pulse" x="338" y="113" width="10" height="181"
              rx="5" fill="url(#drainGrad)" style="animation-delay:.44s"/>
        <rect class="drain-pulse" x="448" y="113" width="10" height="181"
              rx="5" fill="url(#drainGrad)" style="animation-delay:.66s"/>
        <rect class="drain-pulse" x="558" y="113" width="10" height="181"
              rx="5" fill="url(#drainGrad)" style="animation-delay:.88s"/>
      </g>

      <!-- Surcharge arrows -->
      <g class="vertical-arrow" stroke="#FFFFFF" stroke-width="3"
         stroke-linecap="round" stroke-linejoin="round" opacity=".95">
        <path d="M123 77 V99 M116 92 L123 100 L130 92"/>
        <path d="M233 77 V99 M226 92 L233 100 L240 92"/>
        <path d="M343 77 V99 M336 92 L343 100 L350 92"/>
        <path d="M453 77 V99 M446 92 L453 100 L460 92"/>
        <path d="M563 77 V99 M556 92 L563 100 L570 92"/>
      </g>

      <!-- Radial drainage arrows -->
      <g class="radial-arrow" fill="none" stroke="#22D3EE"
         stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"
         filter="url(#softGlow)">
        <path d="M76 151 H108 M99 145 L108 151 L99 157"/>
        <path d="M178 151 H218 M209 145 L218 151 L209 157"/>
        <path d="M288 151 H328 M319 145 L328 151 L319 157"/>
        <path d="M398 151 H438 M429 145 L438 151 L429 157"/>
        <path d="M508 151 H548 M539 145 L548 151 L539 157"/>

        <path d="M76 205 H108 M99 199 L108 205 L99 211"/>
        <path d="M178 205 H218 M209 199 L218 205 L209 211"/>
        <path d="M288 205 H328 M319 199 L328 205 L319 211"/>
        <path d="M398 205 H438 M429 199 L438 205 L429 211"/>
        <path d="M508 205 H548 M539 199 L548 205 L539 211"/>

        <path d="M76 259 H108 M99 253 L108 259 L99 265"/>
        <path d="M178 259 H218 M209 253 L218 259 L209 265"/>
        <path d="M288 259 H328 M319 253 L328 259 L319 265"/>
        <path d="M398 259 H438 M429 253 L438 259 L429 265"/>
        <path d="M508 259 H548 M539 253 L548 259 L539 265"/>
      </g>

      <!-- Vertical drainage arrows: flow upward through PVD -->
      <g class="vertical-drain-arrow" stroke="#67E8F9" stroke-width="3"
         stroke-linecap="round" stroke-linejoin="round"
         filter="url(#softGlow)">
        <path d="M123 307 V288 M116 296 L123 287 L130 296"/>
        <path d="M233 307 V288 M226 296 L233 287 L240 296"/>
        <path d="M343 307 V288 M336 296 L343 287 L350 296"/>
        <path d="M453 307 V288 M446 296 L453 287 L460 296"/>
        <path d="M563 307 V288 M556 296 L563 287 L570 296"/>
      </g>

      <!-- Result bar -->
      <rect x="34" y="321" width="612" height="20" rx="9"
            fill="rgba(15,23,42,.88)" stroke="#24405E"/>
      <rect x="35" y="322" width="{6.10 * uav:.1f}" height="18" rx="8"
            fill="{status_color}" fill-opacity=".82"/>
      <text x="46" y="335" fill="#FFFFFF" font-size="9.5"
            font-weight="800">
        CONSOLIDATION PROGRESS {uav:.1f}%
      </text>
      <text x="475" y="335" fill="{l_color}" font-size="9.5"
            font-weight="800">
        L = {l_text}
      </text>
    </svg>
...
# สร้างภาพเคลื่อนไหวจากค่าที่คำนวณได้จริง
hero_svg = build_dynamic_pvd_svg(
    uav_percent=result.uav * 100.0,
    ur_percent=result.ur * 100.0,
    uv_percent=result.uv * 100.0,
    time_day=time_day,
    target_percent=float(target),
    sand_mat_L=result.sand_mat_L,
    consider_sand_mat=consider_sand_mat,
)
...
# HERO
# =========================================================
# หมายเหตุ: SVG ที่มี <style> และ animation ควร render ผ่าน components.html
# ไม่ควรฝังใน st.markdown โดยตรง เพราะ Markdown อาจตีความ SVG เป็น code block
hero_left, hero_right = st.columns([0.42, 0.58], gap="large")

with hero_left:
    st.markdown(
        """
        <div class="hero" style="min-height:390px;display:flex;align-items:center;">
          <div>
            ...
            <div class="info-strip" style="grid-template-columns:1fr;">
...
with hero_right:
    # สร้างเอกสาร HTML เต็มรูปแบบใน iframe เพื่อให้ SVG และ CSS animation ทำงานแน่นอน
    animated_html = f"""
    <!DOCTYPE html>
    <html lang="th">
    <head>
    ...
    """
    components.html(animated_html, height=405, scrolling=False)
